    ...
    <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
      {data?.map(({id,value,label,icon}, index) => (
        <div key={id} className="bg-mid-100 rounded-2xl md:rounded-[18px] xl:rounded-[23px] p-6 4xl:p-7.5 flex flex-col items-center justify-center border border-grey-border-50 text-center">
          <Text as="h4" className={['text-primary-50 text-2xl xl:text-[28px] 2xl:text-[34px] 4xl:text-[38px] 6xl:text-[43px] font-black mb-1.25',icon ? 'flex items-center':''].join(' ')}>{icon && <Text as="span">{React.createElement(icon)}</Text>}<CountUp value={value} delay={index * 0.12} /></Text>
