                    ...
                    {GenresData?.map(({ id, title, info, thumbnail, category }, index) => (
                        <Activity key={id} mode={activeIndex === index ? 'visible' : 'hidden'}>
                            <div ref={(el) => { contentRefs.current[index] = el }} className="grid grid-cols-1 lg:grid-cols-2 gap-6 items-center">
                                ...
                                {thumbnail && (
                                    <ImageComp src={thumbnail} width={670} height={435} sizes="(max-width: 575px) 352px, (max-width: 991px) 50vw, 670px" className='object-contain h-auto mx-auto max-w-167.5 w-full reflect-below' alt={`image-${category}`} />
                                )}
                            </div>
                        </Activity>
                    ))}
