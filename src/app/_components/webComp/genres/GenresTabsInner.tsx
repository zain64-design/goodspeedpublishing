                    ...
                    {GenresData?.map(({ id, title, info, thumbnail, category }, index) => (
                        <Activity key={id} mode={activeIndex === index ? 'visible' : 'hidden'}>
                            <div ref={(el) => { contentRefs.current[index] = el }} className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                                ...
                                {thumbnail && (
                                    <ImageComp src={thumbnail} width={670} height={435} sizes="(max-width: 575px) 352px, (max-width: 991px) 50vw, 670px" className='object-contain max-h-108.75 mx-auto w-auto reflect-below' alt={`image-${category}`} />
                                )}
                            </div>
                        </Activity>
                    ))}
