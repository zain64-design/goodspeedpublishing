...
//         {data.map((value)=> {
//             const  {id,title,description,newPrice,oldPrice,period,badge,features} = value;
//             return (
//                 <div className={twMerge(`bg-white rounded-[42px] p-12`)} key={id}>
//             <Text as='h4' className='text-black-text-100 font-bold text-[35px] capitalize mb-2.25'>{title}</Text>
//             <Text as='span' className='text-black-text-100 text-[20px] leading-7.25'>{description}</Text>
//             <Text as='h5' className='font-bold text-[70px] leading-15 flex items-end mt-9 mb-3'>{`$${newPrice.toLocaleString()}`} <Text as='small' className='leading-7.25! line-through text-[20px] font-normal opacity-60'>{`$${oldPrice}`}</Text></Text>
//             <Text as='span' className='uppercase text-[16px] tracking-[1.63px]'>{period}</Text>
//             <hr className='border-t border-grey-border-100 mx-auto my-6' />
//             <Text as='ul' className='[&>:not(:last-child)]:mb-3.5'>
//                 {Array.from({length:5}).map((_,index)=> {
//                     return (
//                         <Text key={index} as='li' className='text-black-text-100 text-[20px] leading-7.25'>Manuscript Prep</Text>
//                     )
//                 })}
//             </Text>
//             <hr className='border-t border-grey-border-100 mx-auto my-9' />
//             <CustomBtn label='Order Now' buttonClass='btn-mid font-medium text-[23px] rounded-full h-[76px] items-center justify-center'/>
//             <Text as='h6' className='uppercase font-bold text-[14px] tracking-[1.47px] text-black text-center mt-[38px]'>need more info</Text>
//             <div className="flex items-center justify-center gap-9 mt-3">
//                 <Link href="" className='font-semibold text-[15px] text-black flex flex-col justify-center items-center'><Text as='small' className='capitalize text-grey-text-100 text-[14px] font-medium'>talk to us</Text> +1(646)-389-2410</Link>
//                 <button type='button' className='font-semibold text-[15px] text-black capitalize flex flex-col justify-center items-center'><Text as='small' className='capitalize text-grey-text-100 text-[14px] font-medium'>for more detail</Text> Chat With us</button>
//             </div>
//         </div>
//             )
//         })}
