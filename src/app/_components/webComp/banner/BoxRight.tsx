...
export default function BoxRight({}: Props) {
  const { props: { srcSet: srcSet1 } } = getImageProps({ alt: '', width: 472, height: 269, src: BrEl1 })
...
<ImageComp
    ...
    width={613}
    height={903}
    sizes="(max-width: 575px) 60vw, (max-width: 767px) 45vw, 326px"
    alt="hero"
    fetchPriority="high"
    className="max-w-fit sm:max-w-full max-h-80 sm:max-h-85 md:max-h-100 lg:max-h-100 xl:max-h-120 2xl:max-h-160 4xl:max-h-180 mx-auto block object-contain xl:scale-[1.05]"
/>
    </div>
  )
}