            ...
            <div className="w-[85%] sm:max-w-70 md:max-w-70 lg:max-w-70 xl:max-w-90 2xl:max-w-110 4xl:max-w-130 6xl:max-w-144.25">
                ...
                >
                    {data?.map(({ id, authorThumbnail }) => (
                        <SwiperSlide key={id}>
                            {authorThumbnail && <ImageComp width={577} height={504} sizes="(max-width: 576px) 50vw, (max-width: 991px) 50vw, 577px" src={authorThumbnail} preload className='max-w-full max-h-126 object-contain block mx-auto' alt='author' />}
                        </SwiperSlide>
                    ))}
                </Swiper>
            </div>
